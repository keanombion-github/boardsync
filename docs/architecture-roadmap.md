# Preserved Architecture Reference

Historical build guide retained from the previous agent. Current status and verification live in project-state.md. Root instructions and skill profile are ../Agents.md and ../Skills.md. Historical completion claims below are not fresh runtime evidence.
...
**Phase:** 2a — Core CRUD
**Current Step:** Starting Phase 2a — backend Cards slices + frontend board UI
**Last completed:** Phase 1 complete. CreateBoard, GetBoards, CreateColumn, DeleteColumn slices done. Next.js scaffold with TanStack Query wired to backend. End-to-end data flow verified.
**Blockers:** None.
...
- [x] Scaffold .NET Web API with VSA folder structure
- [x] Create `boardsync` database in local PostgreSQL
- [x] Create Dapper connection factory
- [x] Set up DbUp for migrations, run first migration (users + boards tables)
- [x] Build one slice end-to-end: `CreateBoard` (endpoint → validator → handler → SQL)
- [x] Build `GetBoards` slice
- [x] Scaffold Next.js project with TailwindCSS + shadcn/ui
- [x] Create API client, fetch and display boards from backend
- [x] **Checkpoint:** browser → API → database → browser works
...
The primary purpose of this project is **skill development and deep system design understanding**, not just shipping a product.

**Core goal (user-stated):** Learn deeply by building — understand every layer of the stack well enough to explain design decisions in interviews and real-world scenarios. This includes system design thinking (why we structure things a certain way), not just how to write the code.
...
- System design thinking — scalability, separation of concerns, tradeoffs at every layer
...
- Making every technical decision interview-explainable with real tradeoffs
- Understanding *why* patterns exist, not just how to implement them
...
- After each slice, be able to answer: "Why did you design it this way? What are the tradeoffs?"
- System design questions are treated as seriously as the code itself
